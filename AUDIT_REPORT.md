...
| I-3 | `bls-verifier/Cargo.toml:6-7`, `bls-verifier/Cargo.toml:10` | `crate-type = ["cdylib"]` only — Rust callers cannot use this as a library; `bls-device::primitive::NativePrimitive` therefore re-implements the same code, perpetuating the duplication. Add `"rlib"` so the device can call the same `verify_sync_committee` it ships. **Resolution: wontfix (deliberate divergence).** The cdylib and trait ABIs are intentionally distinct return-code taxonomies; see the M-1 module doc in `bls-device/src/primitive.rs:9-30`. Adding `rlib` and a path dep would invite the very "ABI unification" that PR #48 / PR #43 explicitly rejected (`-2` cdylib vs `-2`/`-5` trait). The duplicated blst calls are a small, audit-bounded cost; the alternative is conflating two published ABIs. |
